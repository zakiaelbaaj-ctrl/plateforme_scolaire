...
function Get-AllTables {
    param([string]$Database, [string]$Host)
    $query = "SELECT table_name FROM information_schema.tables WHERE table_schema='public' AND table_type='BASE TABLE';"
    $tables = psql -U $pgUser -d $Database -h $Host -t -A -c $query
    return $tables
}

# --- Fonction pour récupérer le nombre de lignes pour chaque table ---
function Get-TableCounts {
    param([string]$Database, [string]$Host, [array]$Tables)
    $counts = @()
    foreach ($t in $Tables) {
        $query = "SELECT COUNT(*) FROM public.`"$t`";"
        try {
            $count = psql -U $pgUser -d $Database -h $Host -t -A -c $query
            $counts += [PSCustomObject]@{ Table = $t; Count = [int]$count }
        } catch {
            Write-Warning "Impossible de compter la table $t dans $Database"
            $counts += [PSCustomObject]@{ Table = $t; Count = 0 }
        }
    }
    return $counts
}
...
$allTablesLocal = Get-AllTables -Database $localDb -Host $hostLocal
$allTablesRender = Get-AllTables -Database $renderDb -Host $hostRender
...
$localCounts = Get-TableCounts -Database $localDb -Host $hostLocal -Tables $allTables
$renderCounts = Get-TableCounts -Database $renderDb -Host $hostRender -Tables $allTables
