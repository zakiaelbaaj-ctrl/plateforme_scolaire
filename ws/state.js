...
export function registerSocket(username, ws) {
  if (!username) return;
...
export function unregisterSocket(username, ws) {
  if (!username) return;
...
/** Retourne un snapshot minimal de l'état WebSocket */
export function snapshot() {
  return {
    clients: Array.from(clients.keys()),
    connectedProfs: Array.from(connectedProfs.keys()),
    appelsEnAttente: Array.from(appelsEnAttente.keys()),
    appelsEnCours: Array.from(appelsEnCours.keys()),
    timestamp: Date.now()
  };
}

// Optionnel : export default pour compatibilité
export default {
  registerSocket,
  unregisterSocket,
  snapshot,
